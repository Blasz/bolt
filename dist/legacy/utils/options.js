...
exports.toDependencyTypes = toDependencyTypes;
...
function toDependencyTypes(val, name) {
  if (typeof val !== 'string') {
    throw new Error(`Flag "${name}" must be a string`);
  }
  var types = val.split(',');
  var invalidTypes = types.filter(function(t) {
...
function toDependency(dependencyString) {
  var _dependencyString$spl = dependencyString
      .split('@')
      .filter(function(part) {
        return part !== '';
      }),
    name = _dependencyString$spl[0],
    version = _dependencyString$spl[1];
